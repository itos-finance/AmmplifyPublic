...
contract MakerFacetTest is MultiSetupTest, IUniswapV3FlashCallback {
    ...
    // // ============ Share Inflation Attack Prevention Tests ============

    // /// @notice Test that verifies the share inflation attack is prevented.
    // /// @dev This test reproduces the attack scenario from the vulnerability report:
    // /// 1. Attacker deposits minimum liquidity into empty range
    // /// 2. Attacker donates to pool to inflate share price
    // /// 3. Attacker adjusts position to reduce shares to 1
    // /// 4. Attacker donates again to further inflate share price
    // /// 5. Victim deposits - should either get proper shares or transaction should revert
    // ///
    // /// With the fix in place, virtual offset prevents profitable inflation attacks,
    // /// and minimum share check ensures victim always gets >= 1 shares.
    // function testFirstDepositorDrainPrevented() public {
    //     bytes memory rftData = "";
    ...
    //     // Use a range that's different from the initial pool liquidity
    //     // Ticks must be aligned with tick spacing (60 for fee 3000)
    //     lowTick = 600; // 600 = 10 * 60 ✓
    //     highTick = 720; // 720 = 12 * 60 ✓

    //     // Set current price to the middle of the interval (also aligned)
    //     swapTo(0, TickMath.getSqrtRatioAtTick(660)); // 660 = 11 * 60 ✓

    //     // Fund attacker
    //     vm.startPrank(attacker);
    //     MockERC20(token0).mint(attacker, 10e18);
    //     MockERC20(token1).mint(attacker, 10e18);
    //     MockERC20(token0).approve(diamond, type(uint256).max);
    //     MockERC20(token1).approve(diamond, type(uint256).max);

    //     // Step 1: Attacker deposits minimum compounded maker liquidity
    //     uint256 attackerAssetId = makerFacet.newMaker(
    //         attacker,
    //         poolAddr,
    //         lowTick,
    //         highTick,
    ...
    //     // Step 2: Attacker donates to pool to inflate share price
    //     // Donate 1e18 of each token by transferring directly to pool greater than fee required
    //     MockERC20(token0).transfer(poolAddr, 1e18);
    //     MockERC20(token1).transfer(poolAddr, 1e18);

    //     // Step 3: Adjust attacker position to reduce shares close to 1
    //     // By setting target to a small value, we reduce shares
    //     makerFacet.adjustMaker(attacker, attackerAssetId, 4e14, minSqrtPriceX96, maxSqrtPriceX96, rftData);

    //     // Step 4: Attacker donates again to further inflate share price
    //     MockERC20(token0).transfer(poolAddr, 1e18);
    //     MockERC20(token1).transfer(poolAddr, 1e18);
    //     vm.stopPrank();

    //     // Step 5: Victim attempts to deposit
    //     vm.startPrank(victim);
    //     MockERC20(token0).mint(victim, 2e18);
    //     MockERC20(token1).mint(victim, 2e18);
    //     uint256 balVictim0Before = token0.balanceOf(victim);
    //     uint256 balVictim1Before = token1.balanceOf(victim);
    //     MockERC20(token0).approve(diamond, type(uint256).max);
    //     MockERC20(token1).approve(diamond, type(uint256).max);

    //     // Victim deposits 300e18 liquidity (similar to PoC)
    //     // With the fix, this should either:
    //     // 1. Successfully mint shares (>= 1) for the victim, or
    //     // 2. Revert with MinSharesNotMet if virtual offset protection determines it's unsafe
    //     uint256 victimAssetId = makerFacet.newMaker(
    //         victim,
    //         poolAddr,
    //         lowTick,
    //         highTick,
    //         300e18, // large deposit
    //         true, // compounding
    //         minSqrtPriceX96,
    //         maxSqrtPriceX96,
    //         rftData
    ...
    //     // Verify victim got shares and can withdraw funds
    //     // Query the asset to check shares were minted
    //     (, , , , , uint128 victimLiq, ) = viewFacet.getAssetInfo(victimAssetId);
    //     assertGt(victimLiq, 0, "Victim should have liquidity in their position");

    //     // Remove victim position and verify they can recover their funds
    //     makerFacet.removeMaker(victim, victimAssetId, minSqrtPriceX96, maxSqrtPriceX96, rftData);
    //     vm.stopPrank();

    //     uint256 balVictim0After = token0.balanceOf(victim);
    //     uint256 balVictim1After = token1.balanceOf(victim);

    //     // Calculate loss
    //     uint256 loss0 = balVictim0Before - balVictim0After;
    //     uint256 loss1 = balVictim1Before - balVictim1After;

    //     // With proper fix, victim should recover most/all of their funds
    //     // Some small loss due to fees/rounding is acceptable, but not total loss
    //     // If loss is > 90%, the attack likely succeeded
    //     uint256 deposited0 = 2e18; // Approximate, actual may vary based on liquidity calculation
    //     uint256 deposited1 = 2e18;

    //     // Assert that victim didn't lose all funds (attack prevented)
    //     // Loss should be much less than 50% of deposits
    //     assertLt(loss0, deposited0 / 2, "Victim should not lose majority of token0 funds");
    //     assertLt(loss1, deposited1 / 2, "Victim should not lose majority of token1 funds");

    //     // More importantly, victim should recover something meaningful
    //     assertGt(balVictim0After, 0, "Victim should recover some token0");
    //     assertGt(balVictim1After, 0, "Victim should recover some token1");
    // }

    // // /// @notice Helper function to donate tokens to pool via flash loan callback
    // // /// @dev Implements IUniswapV3FlashCallback to allow donating tokens
    // // function uniswapV3FlashCallback(uint256 fee0, uint256 fee1, bytes calldata data) external override {
    // //     // For donation, we transfer more than what was borrowed
    // //     (uint256 donate0, uint256 donate1) = abi.decode(data, (uint256, uint256));

    // //     MockERC20(token0).transfer(msg.sender, fee0 + donate0);
    // //     MockERC20(token1).transfer(msg.sender, fee1 + donate1);
    // // }

    // /// @notice Test that victim can deposit after attacker's minimum deposit
    // /// @dev Simplified test to verify basic protection. Once virtual offset fix is implemented,
    // ///      this test should pass. Currently may fail with arithmetic underflow due to the vulnerability.
    // function testShareInflationAttackWithMultipleDonations() public {
    //     bytes memory rftData = "";

    //     address victim = makeAddr("victim");
    //     address attacker = makeAddr("attacker");

    //     // Use a different range than the first test to avoid interactions
    //     // Ticks must be aligned with tick spacing (60 for fee 3000)
    //     lowTick = 840; // 840 = 14 * 60 ✓
    //     highTick = 960; // 960 = 16 * 60 ✓
    //     swapTo(0, TickMath.getSqrtRatioAtTick(900)); // 900 = 15 * 60 ✓

    //     // Attacker setup
    //     vm.startPrank(attacker);
    //     MockERC20(token0).mint(attacker, 20e18);
    //     MockERC20(token1).mint(attacker, 20e18);
    //     MockERC20(token0).approve(diamond, type(uint256).max);
    //     MockERC20(token1).approve(diamond, type(uint256).max);

    //     // Attacker deposits minimum liquidity
    //     makerFacet.newMaker(
    //         attacker,
    //         poolAddr,
    //         lowTick,
    ...
    //     // Victim deposits
    //     vm.startPrank(victim);
    //     MockERC20(token0).mint(victim, 10e18);
    //     MockERC20(token1).mint(victim, 10e18);
    //     MockERC20(token0).approve(diamond, type(uint256).max);
    //     MockERC20(token1).approve(diamond, type(uint256).max);
    ...
    //         lowTick,
    //         highTick,
    //         1e7, // smaller, more realistic deposit amount
    //         true, // compounding
    //         minSqrtPriceX96,
    //         maxSqrtPriceX96,
    //         rftData
    ...
    //     // Verify victim position was created successfully
    //     (, , , , , uint128 victimLiq, ) = viewFacet.getAssetInfo(victimAssetId);
    //     assertGt(victimLiq, 0, "Victim position should have liquidity");

    //     // Remove and verify funds recovery
    //     uint256 bal0Before = token0.balanceOf(victim);
    //     uint256 bal1Before = token1.balanceOf(victim);

    //     makerFacet.removeMaker(victim, victimAssetId, minSqrtPriceX96, maxSqrtPriceX96, rftData);

    //     uint256 bal0After = token0.balanceOf(victim);
    //     uint256 bal1After = token1.balanceOf(victim);

    //     // Key test: Victim should recover some funds (proving they got shares)
    //     // Without the fix, victim would get 0 shares and lose all funds
    //     assertTrue(
    //         bal0After > bal0Before || bal1After > bal1Before,
    //         "Victim should recover funds, proving they received shares (attack prevented)"
    //     );
    //     vm.stopPrank();
    // }
    ...
    function testFirstDepositorDrain() public {
        ...
        console2.log("victim balance loss: ");
        console2.log(balVictim0, balVictim0After, balVictim1, balVictim1After);
    }
}
