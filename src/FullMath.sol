...
library FullMath {
    ...
    function mulX256(uint256 a, uint256 b, bool roundUp) internal pure returns (uint256) {
        (uint256 bot, uint256 top) = FullMath.mul512(a, b);
        if (roundUp && bot > 0) top += 1;
        return top;
    }

    /// Multiply two numbers and divide out by 64 bits
    /// @dev Be careful when using this. If there is overflow you won't be warned.
    function mulX64(uint256 a, uint256 b, bool roundUp) internal pure returns (uint256 res) {
        (uint256 bot, uint256 top) = FullMath.mul512(a, b);
        uint256 modmax = 1 << 64;
        assembly {
