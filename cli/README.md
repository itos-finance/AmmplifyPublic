...
| `AMMPLIFY_CHAIN_ID` | Yes | — | Target chain ID (e.g. `143` for Monad Mainnet) |
...
| `AMMPLIFY_ADDRESSES_FILE` | No | `./addresses/uniswapv3.json` | Path to contract addresses (relative to `cli/`) |

The CLI reads contract addresses from a JSON file with this flat structure:
...
```json
{
  ...
  "nfpm": "0x...",
  "router": "0x...",
  "pools": { "USDC_WETH_3000": "0x..." }
}
```

Two address files are bundled: `addresses/uniswapv3.json` (default) and `addresses/capricorn.json`.
