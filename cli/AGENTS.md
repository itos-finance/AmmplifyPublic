...
├── config.ts                  # Env loading (.env), token resolution, deployed address loading
...
All Ammplify protocol calls go through the diamond address (retrieved via `getDiamondAddress()`). The diamond delegates to facets based on function selector, but from the CLI's perspective it's a single address. Use the facet-specific ABI (`IMakerAbi`, `ITakerAbi`, `IViewAbi`, `IAdminAbi`) — viem only needs the function signature, not the actual facet address.
...
- Token symbols: `"USDC"`, `"WETH"` (case-insensitive, matched against JSON keys)
- Raw hex addresses: `"0x..."`

Returns `{ address, symbol, decimals }`. Token decimals come from the address JSON (`{ address, decimals }` per token).

## Deployed Addresses

Loaded from `addresses/uniswapv3.json` (path configurable via `AMMPLIFY_ADDRESSES_FILE` env var). Use `addresses/capricorn.json` for the Capricorn protocol. Cached as a singleton after first load in `config.ts`.

Structure (flat schema):
```
{ network, chainId, tokens: { SYMBOL: { address, decimals } }, diamond, opener?, decomposer, factory, nfpm, router, pools: { PAIR_FEE: address } }
```

The `AMMPLIFY_PROTOCOL` env var controls which address file Foundry scripts use (defaults to `uniswapv3`).
...
| `AMMPLIFY_ADDRESSES_FILE` | No | Path to addresses JSON (defaults to `./addresses/uniswapv3.json`) |
