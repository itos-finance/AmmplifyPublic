...
contract SwapToPriceSimpleRouter is Script {
    ...
    function loadEnvironment() public {
        ...
        env.deployer = vm.envAddress("DEPLOYER_PUBLIC_KEY");
        env.uniswapFactory = json.readAddress(".factory");
        env.simpleSwapRouter = json.readAddress(".router");
        env.jsonPath = path;
    ...
    /**
     * @notice Get pool address by key from addresses JSON
     ...
     *      - POOL_KEY: The pool key from addresses JSON (e.g., "USDC_WETH_3000")
     ...
     * @param poolKey The pool key from addresses JSON
    ...
    /**
     * @notice List all available pools from addresses JSON
     ...
     * function lists the known pools. Users can check addresses JSON
     * for the full list.
     */
    ...
    /**
     * @notice Get all pool keys from addresses JSON
