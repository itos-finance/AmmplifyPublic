...
contract CreateUniV3Position is AmmplifyPositions {
    ...
    function run() public override {
        ...
        console2.log("Uniswap NFPM address:", env.nfpm);

        // Get current pool state
        printPoolState(getPoolAddress("USDC_WETH_3000"));

        // Fund the account with tokens (if using mock tokens)
        fundAccount(deployer, 1000e6, 1e18, getPoolAddress("USDC_WETH_3000")); // 1000 USDC, 1 WETH
        ...
        INonfungiblePositionManager nfpm = INonfungiblePositionManager(env.nfpm);
    ...
    function createBasicPosition(
        ...
        (, int24 currentTick, , , , , ) = IUniswapV3Pool(getPoolAddress("USDC_WETH_3000")).slot0();
    ...
    function setupUniswapApprovals(uint256 amount) public {
        if (env.nfpm != address(0)) {
            ...
            tokens[0] = getTokenAddress("USDC");
            tokens[1] = getTokenAddress("WETH");
            address[] memory spenders = new address[](1);
            spenders[0] = env.nfpm;
            setupApprovals(tokens, spenders, amount);
            console2.log("Approved Uniswap NFPM contract:", env.nfpm);
        }
    }
