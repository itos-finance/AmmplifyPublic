 ...
 * - FACTORY_ADDRESS: The Uniswap V3 Factory address (or will read from addresses JSON)
...
contract CreatePool is Script {
    ...
    function _getFactoryAddress() internal view returns (address) {
        ...
        if (stdJson.keyExists(json, ".factory")) {
            address factoryAddr = stdJson.readAddress(json, ".factory");
            require(factoryAddr != address(0), "Factory address not found in addresses JSON");
            return factoryAddr;
        }

        revert(
            "Factory address not found. Set FACTORY_ADDRESS env var or ensure .factory exists in addresses JSON"
        );
    }
