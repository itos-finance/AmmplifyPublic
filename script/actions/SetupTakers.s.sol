 ...
 * @dev   - POOL_KEY: Pool key from addresses JSON (default: "USDC_WETH_3000")
...
contract SetupTakers is AmmplifyPositions {
    ...
    // getPoolAddress is inherited from AmmplifyPositions
    ...
    /**
     * @notice Get token decimals by looking up the address in the loaded JSON
     */
    function _getTokenDecimals(address token) internal view returns (uint8) {
        string memory json = env.jsonRaw;
        string[7] memory tokenSymbols = ["USDC", "USDT0", "WETH", "WMON", "WBTC", "CHOG", "shMON"];
