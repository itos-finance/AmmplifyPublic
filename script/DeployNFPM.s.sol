 ...
 * - Ensure addresses JSON exists with uniswap.factory address (preferred)
 ...
 *
 * Note: The script will automatically use the factory address from addresses JSON
...
contract DeployNFPM is Script {
    ...
    /**
     * @notice Load configuration from addresses JSON and environment variables
     */
    function _loadConfiguration() internal {
        ...
        // Load factory address from addresses JSON
        ...
        try vm.readFile(addrPath) returns (string memory jsonData) {
            factoryAddress = stdJson.readAddress(jsonData, ".factory");
            console.log("Using factory from addresses JSON at:", factoryAddress);
        } catch {
            ...
            try vm.envAddress("FACTORY") returns (address factoryAddr) {
                ...
                console.log("No factory found in addresses JSON or environment, will deploy new factory");
